...
export interface InventoryOverview {
  locations: Array<
    Pick<InventoryLocation, 'id' | 'code' | 'name' | 'locationType' | 'isDefault'> & {
      _count: { stockUnits: number };
    }
  >;
  physicalItems: Array<{
...
export interface InventoryItem {
  ...
  lastRental: {
    ...
    reservation: { booking: { id: string; bookingNumber: string } };
  } | null;
  nextRental: {
...
export type InventoryStockState = 'AVAILABLE' | 'LOW_STOCK' | 'UNAVAILABLE' | 'UNCONFIGURED';
...
export interface InventorySku {
  ...
  operationallyAvailableCount: number;
...
export interface InventoryBlock {
  ...
  reason: string | null;
  canDelete: boolean;
  owner: 'MANUAL' | 'SERVICE_ORDER' | 'INSPECTION';
  product: { id: string; name: string } | null;
...
export interface InventoryCountDetail extends InventoryCountSummary {
  observations: Array<{
    ...
    identityMatch: 'ASSET_CODE' | 'BARCODE' | 'UNKNOWN';
    isDuplicate: boolean;
    stockUnit: { id: string; assetCode: string; barcode: string | null } | null;
  }>;
  items: Array<{
    id: string;
    ...
    recordedDisposition: StockUnitDisposition;
    recordedOperationalState: StockUnitOperationalState;
    recordedLocation: { id: string; code: string; name: string };
    stockUnit: {
      id: string;
      assetCode: string;
      barcode: string | null;
...
export const inventoryApi = {
  ...
  listSkus: async (params?: InventorySkuQuery): Promise<PaginatedResponse<InventorySku>> => {
    const { data } = await apiClient.get<PaginatedResponse<InventorySku>>('/owner/inventory/skus', {
      params,
    });
    return data;
  },
  ...
  registerItemBatch: async (
    ...
    unwrap(
      await apiClient.post<
        ApiResponse<{ replayed: boolean; units: Array<{ id: string; assetCode: string }> }>
      >(`/owner/variant-sizes/${variantSizeId}/stock-units/batch`, payload),
    ),

  listLocations: async (includeInactive = false): Promise<InventoryLocation[]> =>
    unwrap(
      await apiClient.get<ApiResponse<InventoryLocation[]>>('/owner/inventory/locations', {
  ...
  createLocation: async (payload: InventoryLocationInput): Promise<InventoryLocation> =>
    unwrap(
      await apiClient.post<ApiResponse<InventoryLocation>>('/owner/inventory/locations', payload),
    ),

  updateLocation: async (
    locationId: string,
    payload: Partial<Omit<InventoryLocationInput, 'code'>> & { isActive?: boolean },
  ): Promise<InventoryLocation> =>
    unwrap(
      await apiClient.patch<ApiResponse<InventoryLocation>>(
        `/owner/inventory/locations/${locationId}`,
        payload,
      ),
    ),
  ...
  setDefaultLocation: async (locationId: string): Promise<InventoryLocation> =>
    unwrap(
      await apiClient.post<ApiResponse<InventoryLocation>>(
        `/owner/inventory/locations/${locationId}/default`,
      ),
    ),

  listPolicies: async (): Promise<AvailabilityPolicy[]> =>
    unwrap(
      await apiClient.get<ApiResponse<AvailabilityPolicy[]>>(
        '/owner/inventory/availability-policies',
      ),
    ),

  upsertPolicy: async (payload: AvailabilityPolicyInput): Promise<AvailabilityPolicy> =>
    unwrap(
      await apiClient.put<ApiResponse<AvailabilityPolicy>>(
        '/owner/inventory/availability-policies',
        payload,
      ),
    ),

  deactivatePolicy: async (
    policyId: string,
    expectedVersion: number,
  ): Promise<AvailabilityPolicy> =>
    unwrap(
      await apiClient.delete<ApiResponse<AvailabilityPolicy>>(
        `/owner/inventory/availability-policies/${policyId}`,
        { params: { expectedVersion } },
      ),
    ),

  resolvePolicy: async (params: {
    productId: string;
    variantSizeId: string;
    locationId: string;
  }): Promise<{ target: Record<string, unknown>; effective: EffectiveAvailabilityPolicy }> =>
    unwrap(
      await apiClient.get<
        ApiResponse<{ target: Record<string, unknown>; effective: EffectiveAvailabilityPolicy }>
      >('/owner/inventory/availability-policies/resolved', { params }),
    ),
  ...
  listBlocks: async (params?: {
    ...
    to?: string;
    activeOnly?: boolean;
  }): Promise<PaginatedResponse<InventoryBlock>> => {
    const { data } = await apiClient.get<PaginatedResponse<InventoryBlock>>(
  ...
  previewBlock: async (payload: InventoryBlockInput): Promise<InventoryBlockPreview> =>
    unwrap(
      await apiClient.post<ApiResponse<InventoryBlockPreview>>(
        '/owner/inventory/blocks/preview',
        payload,
      ),
    ),

  createBlock: async (
    payload: InventoryBlockInput,
  ): Promise<{ block: InventoryBlock; preview: InventoryBlockPreview }> =>
    unwrap(
      await apiClient.post<ApiResponse<{ block: InventoryBlock; preview: InventoryBlockPreview }>>(
        '/owner/inventory/blocks',
        payload,
      ),
    ),
  ...
  listCounts: async (
    params?: InventoryMovementQuery,
  ): Promise<PaginatedResponse<InventoryCountSummary>> => {
    const { data } = await apiClient.get<PaginatedResponse<InventoryCountSummary>>(
  ...
  getCount: async (countSessionId: string): Promise<InventoryCountDetail> =>
    unwrap(
      await apiClient.get<ApiResponse<InventoryCountDetail>>(
        `/owner/inventory/counts/${countSessionId}`,
      ),
    ),

  reconcileCount: async (
    payload: ReconcileInventoryCountInput,
  ): Promise<{ replayed: boolean; session: InventoryCountDetail }> =>
    unwrap(
      await apiClient.post<ApiResponse<{ replayed: boolean; session: InventoryCountDetail }>>(
        '/owner/inventory/counts/reconcile',
        payload,
      ),
    ),
  ...
  listTransfers: async (status?: InventoryTransferStatus): Promise<InventoryTransfer[]> =>
    unwrap(
      await apiClient.get<ApiResponse<InventoryTransfer[]>>('/owner/inventory/transfers', {
        params: { status },
      }),
    ),
  ...
  getTransfer: async (transferId: string): Promise<InventoryTransfer> =>
    unwrap(
      await apiClient.get<ApiResponse<InventoryTransfer>>(
        `/owner/inventory/transfers/${transferId}`,
      ),
    ),

  createTransfer: async (payload: Record<string, unknown>): Promise<InventoryTransfer> =>
    unwrap(
      await apiClient.post<ApiResponse<InventoryTransfer>>('/owner/inventory/transfers', payload),
    ),
  ...
  transferAction: async (
    ...
    unwrap(
      await apiClient.post<ApiResponse<InventoryTransfer>>(
        `/owner/inventory/transfers/${transferId}/${action}`,
  ...
  receiveTransfer: async (
    ...
    unwrap(
      await apiClient.post<ApiResponse<InventoryTransfer>>(
        `/owner/inventory/transfers/${transferId}/receive`,
