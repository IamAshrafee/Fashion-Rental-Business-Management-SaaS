...
export const analyticsApi = {
  downloadExport: async (
    type: 'bookings' | 'customers' | 'inventory' | 'payments' | 'recovery',
    params?: { from?: string; to?: string },
  ) => {
    const response = await apiClient.get<Blob>(`/owner/analytics/export/${type}`, {
      params,
      responseType: 'blob',
    });
    const disposition = response.headers['content-disposition'] as string | undefined;
    const filename = disposition?.match(/filename="?([^";]+)"?/)?.[1] || `${type}-export.csv`;
  ...
  getRevenueSeries: async (params?: { from?: string; to?: string; groupBy?: string }) => {
    const { data } = await apiClient.get<ApiResponse<RevenueSeries>>('/owner/analytics/revenue', {
      params,
    });
    return data;
  },
  ...
  getStorefrontSummary: async (params?: { from?: string; to?: string; productId?: string }) => {
    const { data } = await apiClient.get<ApiResponse<StorefrontTrafficSummary>>(
      '/owner/analytics/traffic/summary',
      { params },
    );
    return data;
  },
  ...
  getFunnelMetrics: async (params?: { from?: string; to?: string; productId?: string }) => {
    const { data } = await apiClient.get<ApiResponse<TrafficFunnel>>(
      '/owner/analytics/traffic/funnel',
      { params },
    );
    return data;
  },
  ...
  getTopViewedProducts: async (params?: { from?: string; to?: string }) => {
    const { data } = await apiClient.get<ApiResponse<TopViewedProduct[]>>(
      '/owner/analytics/traffic/top-products',
      { params },
    );
    return data;
  },
  ...
  getMarketingAttribution: async (params?: { from?: string; to?: string }) => {
    const { data } = await apiClient.get<ApiResponse<AttributionSource[]>>(
      '/owner/analytics/traffic/attribution',
      { params },
    );
    return data;
  },
