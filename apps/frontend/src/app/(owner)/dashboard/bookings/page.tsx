...
export default function BookingsPage() {
  ...
  return <div className="space-y-6 pb-10">
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      <Card><CardHeader className="pb-2"><CardTitle className="flex items-center justify-between text-sm font-medium"><span>Awaiting review</span><ClipboardCheck className="size-4 text-muted-foreground" /></CardTitle></CardHeader><CardContent><button className="text-3xl font-semibold" onClick={() => update({ queue: 'REQUEST' })}>{stats.data?.queueCounts.REQUEST ?? '—'}</button></CardContent></Card>
      <Card><CardHeader className="pb-2"><CardTitle className="flex items-center justify-between text-sm font-medium"><span>Needs item assignment</span><PackageCheck className="size-4 text-muted-foreground" /></CardTitle></CardHeader><CardContent><button className="text-3xl font-semibold" onClick={() => update({ queue: 'ASSIGNMENT' })}>{stats.data?.queueCounts.ASSIGNMENT ?? '—'}</button></CardContent></Card>
      <Card><CardHeader className="pb-2"><CardTitle className="flex items-center justify-between text-sm font-medium"><span>Handoff work</span><CalendarIcon className="size-4 text-muted-foreground" /></CardTitle></CardHeader><CardContent><button className="text-3xl font-semibold" onClick={() => update({ queue: 'HANDOFF' })}>{stats.data?.queueCounts.HANDOFF ?? '—'}</button><p className="text-xs text-muted-foreground">{stats.data?.queueCounts.RETURN_DUE ?? 0} returns due</p></CardContent></Card>
