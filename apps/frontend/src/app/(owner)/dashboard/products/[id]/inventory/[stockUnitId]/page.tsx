...
const INSPECTION_TYPES: StockUnitInspectionType[] = [
  'PRE_RENTAL',
  'RETURN',
  'PERIODIC',
  'SERVICE_COMPLETION',
];
const DECISIONS: StockUnitInspectionDecision[] = [
  'AVAILABLE',
  'CLEANING',
  'WASHING',
  'REPAIR',
...
const SERVICE_TYPES: InventoryServiceOrderType[] = [
  'PREPARATION',
  'CLEANING',
  'WASHING',
  'REPAIR',
...
const PRESENCE_OPTIONS: StockUnitComponentPresence[] = [
  'PRESENT',
  'MISSING',
  'DAMAGED',
  'NOT_APPLICABLE',
...
function apiErrorMessage(error: unknown, fallback: string) {
  const message = (
    error as {
      response?: {
        data?: { message?: string | { message?: string }; error?: { message?: string } };
      };
    }
  )?.response?.data;
...
function availabilityVariant(unit: StockUnitOperations['stockUnit']) {
  ...
  if (unit.disposition === 'QUARANTINED' || unit.operationalState !== 'AVAILABLE')
...
function LifecyclePanel({
  ...
  const transition = useMutation({
    mutationFn: () =>
      inventoryOperationsApi.transition(unit.id, {
        targetState: state as StockUnitOperationalState,
        reason: stateReason,
        idempotencyKey: commandKey('unit-state'),
      }),
    onSuccess: async () => {
      setState('');
      setStateReason('');
  ...
  const changeDisposition = useMutation({
    mutationFn: () =>
      inventoryOperationsApi.changeDisposition(unit.id, {
        targetDisposition: disposition as StockUnitDisposition,
        reason: dispositionReason,
        idempotencyKey: commandKey('unit-disposition'),
      }),
    onSuccess: async () => {
      setDisposition('');
      setDispositionReason('');
      await refresh();
      toast.success('Unit disposition updated');
    },
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Where the item is in its current rental or care workflow.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          ...
          {OPERATIONAL_TRANSITIONS[unit.operationalState].length ? (
            <>
              <Select
                value={state}
                onValueChange={(value) => setState(value as StockUnitOperationalState)}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Move to…" />
                </SelectTrigger>
                <SelectContent>
                  {OPERATIONAL_TRANSITIONS[unit.operationalState].map((item) => (
              ...
              />
              <Button
                disabled={!state || !stateReason.trim() || transition.isPending}
                onClick={() => transition.mutate()}
              >
                {transition.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Apply
                state
              </Button>
            </>
          ) : (
            <p className="text-sm text-muted-foreground">
              No operational transitions are allowed from this state.
            </p>
          )}
        </CardContent>
      </Card>
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Asset disposition</CardTitle>
          <CardDescription>
            Administrative availability: active, quarantined, lost, or retired.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          ...
          {DISPOSITION_TRANSITIONS[unit.disposition].length ? (
            <>
              <Select
                value={disposition}
                onValueChange={(value) => setDisposition(value as StockUnitDisposition)}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Change disposition…" />
                </SelectTrigger>
                <SelectContent>
                  {DISPOSITION_TRANSITIONS[unit.disposition].map((item) => (
                    <SelectItem key={item} value={item}>
                      {label(item)}
                    </SelectItem>
                  ))}
              ...
              <Button
                variant={
                  disposition === 'LOST' || disposition === 'RETIRED' ? 'destructive' : 'default'
                }
                disabled={!disposition || !dispositionReason.trim() || changeDisposition.isPending}
                onClick={() => changeDisposition.mutate()}
              >
                {changeDisposition.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                Apply disposition
              </Button>
            </>
          ) : (
            <p className="text-sm text-muted-foreground">
              {unit.disposition === 'RETIRED'
                ? 'Retirement is final. The unit history remains available for audit.'
                : 'Create and complete an inspection with an Available decision to reactivate this item.'}
            </p>
          )}
        </CardContent>
      </Card>
...
function CreateInspectionDialog({
  ...
  const create = useMutation({
    ...
    onSuccess: async () => {
      ...
      setNotes('');
      await refresh();
      toast.success('Inspection draft created');
    },
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Start item inspection</DialogTitle>
          <DialogDescription>
            The draft blocks return and periodic workflows until a decision is recorded.
          </DialogDescription>
        </DialogHeader>
        <div className="grid gap-4 py-2">
          <div className="grid gap-2">
            ...
            <Select
              ...
              onValueChange={(value) => setInspectionType(value as StockUnitInspectionType)}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
          ...
          {requiresAssignment && (
            <div className="grid gap-2">
              <Label>Rental assignment</Label>
              <Select value={assignmentId} onValueChange={setAssignmentId}>
                <SelectTrigger>
                  <SelectValue placeholder="Select assignment" />
                </SelectTrigger>
                <SelectContent>
                  {data.stockUnit.assignments.map((item) => (
                    <SelectItem key={item.id} value={item.id}>
                      Booking {item.reservation.bookingId.slice(0, 8)} ·{' '}
                      {item.blockedStartDate.slice(0, 10)} to {item.blockedEndDate.slice(0, 10)}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
              {!data.stockUnit.assignments.length && (
          ...
          {inspectionType === 'SERVICE_COMPLETION' && (
            <div className="grid gap-2">
              <Label>Completed service work</Label>
              <Select value={serviceOrderId} onValueChange={setServiceOrderId}>
                ...
                <SelectContent>
                  {completedServices.map((item) => (
                    <SelectItem key={item.id} value={item.id}>
                      {label(item.serviceType)} · {item.completedAt?.slice(0, 10)}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
              {!completedServices.length && (
                <p className="text-xs text-destructive">
                  No completed service order is awaiting inspection.
                </p>
              )}
            </div>
          )}
...
function CompleteInspectionDialog({
  ...
  definitions: SetComponentDefinition[];
  stockUnitId: string;
  refresh: () => Promise<void>;
}) {
  const [open, setOpen] = useState(false);
  const [conditionAfter, setConditionAfter] = useState<StockConditionGrade>(
  ...
  const [checks, setChecks] = useState<Record<string, CheckDraft>>(() =>
    Object.fromEntries(
      definitions.map((item) => [
        item.id,
        { result: 'PASS', observedQuantity: item.requiredQuantity, notes: '' },
      ]),
    ),
  );
  ...
  const complete = useMutation({
    mutationFn: async () => {
      const mediaFiles = files.length
        ? await inventoryOperationsApi.uploadInspectionMedia(stockUnitId, files)
        : [];
          ...
          ? [
              {
                ...
                isAvailabilityBlocking: blocking,
              },
            ]
          : undefined,
        media: mediaFiles.map((file) => ({
          ...file,
          purpose: inspection.inspectionType === 'RETURN' ? 'POST_RETURN' : 'PRE_RENTAL',
        })),
    ...
    onSuccess: async () => {
      setOpen(false);
      await refresh();
      toast.success('Inspection completed and lifecycle updated');
    },
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Complete {label(inspection.inspectionType)} inspection</DialogTitle>
          <DialogDescription>
            Record observed condition, set completeness, evidence, and the next operational
            decision.
          </DialogDescription>
        </DialogHeader>
        <div className="grid gap-5 py-2">
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="grid gap-2">
              ...
              <Select
                ...
                onValueChange={(value) => setConditionAfter(value as StockConditionGrade)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div className="grid gap-2">
              ...
              <Select
                value={decision}
                onValueChange={(value) => setDecision(value as StockUnitInspectionDecision)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
          ...
          {definitions.length > 0 && (
            <div className="space-y-3">
              <Label>Required set checklist</Label>
              {definitions.map((definition) => {
                const value = checks[definition.id] || {
                  result: 'PASS' as const,
                  observedQuantity: definition.requiredQuantity,
                  notes: '',
                };
                return (
                  <div
                    key={definition.id}
                    className="grid gap-3 rounded-md border p-3 sm:grid-cols-[1fr_120px_170px]"
                  >
                    <div>
                      <p className="text-sm font-medium">
                        {definition.name} × {definition.requiredQuantity}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {definition.inspectionGuidance ||
                    ...
                    <Input
                      aria-label={`${definition.name} observed quantity`}
                      type="number"
                      min={0}
                      value={value.observedQuantity}
                      onChange={(event) =>
                        setChecks((current) => ({
                          ...current,
                          [definition.id]: {
                            ...value,
                            observedQuantity: Math.max(0, Number(event.target.value) || 0),
                          },
                        }))
                      }
                    />
                    <Select
                      ...
                      onValueChange={(result) =>
                        setChecks((current) => ({
                          ...current,
                          [definition.id]: { ...value, result: result as InspectionCheckResult },
                        }))
                      }
                    >
                      <SelectTrigger>
          ...
          <div className="grid gap-2">
            <Label>Customer liability note (internal)</Label>
            <Textarea
              value={liabilityNote}
              onChange={(event) => setLiabilityNote(event.target.value)}
              placeholder="How customer responsibility was assessed"
            />
          </div>
          <div className="grid gap-2">
            <Label htmlFor={`inspection-media-${inspection.id}`}>Private evidence photos</Label>
            <Input
              id={`inspection-media-${inspection.id}`}
              type="file"
            ...
            />
            <p className="text-xs text-muted-foreground">
              Up to 10 images. Stored privately and linked to this inspection.
            </p>
          </div>
          <div className="space-y-3 rounded-md border p-3">
            <div className="flex items-center gap-2">
              <Checkbox
                id={`record-issue-${inspection.id}`}
                checked={recordIssue}
                onCheckedChange={(checked) => setRecordIssue(checked === true)}
              />
              <Label htmlFor={`record-issue-${inspection.id}`}>
                Record an issue found during inspection
              </Label>
            </div>
            {recordIssue && (
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="grid gap-2">
                  ...
                  <Input
                    ...
                    placeholder="Stain, tear, missing accessory…"
                  />
                </div>
                <div className="grid gap-2">
                  ...
                  <Select
                    value={severity}
                    onValueChange={(value) => setSeverity(value as StockUnitIssueSeverity)}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      {(
                        ['MINOR', 'MODERATE', 'SEVERE', 'CRITICAL'] as StockUnitIssueSeverity[]
                      ).map((item) => (
                        <SelectItem key={item} value={item}>
                          {label(item)}
                        </SelectItem>
                ...
                <div className="grid gap-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setResponsibility(value as StockUnitIssueResponsibility)
                    }
                  >
                    <SelectTrigger>
                      <SelectValue />
                    ...
                    <SelectContent>
                      {(
                        [
                          ...
                          'NORMAL_WEAR',
                          'UNKNOWN',
                        ] as StockUnitIssueResponsibility[]
                      ).map((item) => (
                        <SelectItem key={item} value={item}>
                          {label(item)}
                        </SelectItem>
                ...
                <div className="flex items-end gap-2 pb-2">
                  <Checkbox
                    id={`blocking-${inspection.id}`}
                    checked={blocking}
                    onCheckedChange={(checked) => setBlocking(checked === true)}
                  />
                  <Label htmlFor={`blocking-${inspection.id}`}>Blocks future rental</Label>
                </div>
        ...
        <DialogFooter>
          ...
          <Button disabled={!validIssue || complete.isPending} onClick={() => complete.mutate()}>
            {complete.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Complete and
            apply decision
          </Button>
        </DialogFooter>
      </DialogContent>
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="font-medium">Inspections</h2>
          <p className="text-sm text-muted-foreground">
          ...
          <Card key={inspection.id}>
            <CardHeader className="pb-3">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <CardTitle className="text-base">{label(inspection.inspectionType)}</CardTitle>
                  <CardDescription>
                    {formatDate(inspection.createdAt)} · {inspection.inspectedBy.fullName}
                  </CardDescription>
                </div>
                <div className="flex gap-2">
                  <Badge variant={inspection.status === 'DRAFT' ? 'secondary' : 'outline'}>
                    {label(inspection.status)}
                  </Badge>
                  {inspection.decision && <Badge>{label(inspection.decision)}</Badge>}
                </div>
              </div>
            </CardHeader>
            <CardContent className="space-y-3">
              <div className="grid gap-2 text-sm sm:grid-cols-3">
                <p>
                  Before: <strong>{label(inspection.conditionBefore)}</strong>
                </p>
                <p>
                  After:{' '}
                  <strong>
                    {inspection.conditionAfter ? label(inspection.conditionAfter) : 'Pending'}
                  </strong>
                </p>
                <p>
                  Checks: <strong>{inspection.checks.length}</strong>
                </p>
              </div>
              {inspection.notes && (
              ...
              {inspection.status === 'DRAFT' && (
                <CompleteInspectionDialog
                  ...
                  definitions={data.stockUnit.variantSize.setComponentDefinitions}
                  stockUnitId={data.stockUnit.id}
                  refresh={refresh}
                />
              )}
            </CardContent>
...
function ResolveIssueDialog({
  ...
  const resolve = useMutation({
    mutationFn: () =>
      inventoryOperationsApi.resolveIssue(issue.id, {
        resolutionNotes: notes,
        waive,
        idempotencyKey: commandKey('issue-resolve'),
      }),
    onSuccess: async () => {
      setOpen(false);
      await refresh();
      toast.success(waive ? 'Issue waived' : 'Issue resolved');
    },
    onError: (error) => toast.error(apiErrorMessage(error, 'Could not resolve issue')),
  });
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Service work linked to this issue must be completed first.
          </DialogDescription>
        </DialogHeader>
        <div className="grid gap-4 py-2">
          <Textarea
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <Label htmlFor={`waive-${issue.id}`}>Waive instead of resolve</Label>
          </div>
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={() => setOpen(false)}>
          ...
          <Button disabled={!notes.trim() || resolve.isPending} onClick={() => resolve.mutate()}>
            Close issue
          </Button>
        </DialogFooter>
      </DialogContent>
  ...
  return (
    <div className="space-y-3">
      {!data.issues.length ? (
        <Card>
          <CardContent className="p-8 text-center text-sm text-muted-foreground">
            No condition or loss issues recorded.
          </CardContent>
        </Card>
      ) : (
            ...
            <CardContent className="flex flex-wrap items-start justify-between gap-4 p-5">
              <div className="space-y-2">
                <div className="flex flex-wrap gap-2">
                  <Badge variant={issue.isAvailabilityBlocking ? 'destructive' : 'secondary'}>
                    {label(issue.severity)}
                  </Badge>
                  <Badge variant="outline">{label(issue.status)}</Badge>
                  <Badge variant="outline">{label(issue.responsibility)}</Badge>
                </div>
                <div>
                  <p className="font-medium">{issue.issueType}</p>
                  <p className="max-w-2xl text-sm text-muted-foreground">{issue.description}</p>
                </div>
                <p className="text-xs text-muted-foreground">
                  Reported {formatDate(issue.createdAt)}
                  {issue.reportedBy ? ` by ${issue.reportedBy.fullName}` : ''}
                </p>
                {issue.resolutionNotes && (
                  <p className="text-sm">Resolution: {issue.resolutionNotes}</p>
                )}
              </div>
              {(issue.status === 'OPEN' || issue.status === 'IN_SERVICE') && (
                <ResolveIssueDialog issue={issue} refresh={refresh} />
              )}
            </CardContent>
          </Card>
        ))
...
function CreateServiceDialog({
  ...
  const create = useMutation({
    mutationFn: () =>
      inventoryOperationsApi.createServiceOrder(data.stockUnit.id, {
        serviceType,
        issueId: issueId === 'none' ? undefined : issueId,
        providerName: providerName.trim() || undefined,
        expectedCompletionAt: expectedCompletionAt || undefined,
        notes: notes.trim() || undefined,
        isAvailabilityBlocking: true,
        idempotencyKey: commandKey('service-create'),
      }),
    onSuccess: async () => {
      setOpen(false);
      await refresh();
    ...
    onError: (error) => toast.error(apiErrorMessage(error, 'Could not create service order')),
  });
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Schedule item care</DialogTitle>
          <DialogDescription>
            The item is blocked from availability while blocking service work is active.
          </DialogDescription>
        </DialogHeader>
        <div className="grid gap-4 py-2">
          <div className="grid gap-2">
            ...
            <Select
              value={serviceType}
              onValueChange={(value) => setServiceType(value as InventoryServiceOrderType)}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
          ...
          <div className="grid gap-2">
            ...
            <Select value={issueId} onValueChange={setIssueId}>
              ...
              <SelectContent>
                ...
                {openIssues.map((item) => (
                  <SelectItem key={item.id} value={item.id}>
                    {item.issueType} · {label(item.severity)}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
...
function CompleteServiceDialog({
  ...
  const complete = useMutation({
    mutationFn: () =>
      inventoryOperationsApi.completeServiceOrder(order.id, {
        completionOutcome: outcome,
        requiresInspection: canSkipInspection ? requiresInspection : true,
        idempotencyKey: commandKey('service-complete'),
      }),
    onSuccess: async () => {
      setOpen(false);
      await refresh();
      toast.success('Service work completed');
    },
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Complete service work</DialogTitle>
          <DialogDescription>
            {canSkipInspection
              ? 'You may return routine care directly to availability, or require a verification inspection.'
              : 'Repair, alteration, and maintenance always require a completion inspection.'}
          </DialogDescription>
        </DialogHeader>
        <div className="grid gap-4 py-2">
          <Textarea
            ...
            placeholder="Work completed and observed outcome"
          />
          {canSkipInspection && (
            <div className="flex items-center gap-2">
              <Checkbox
                id={`requires-inspection-${order.id}`}
                checked={requiresInspection}
                onCheckedChange={(checked) => setRequiresInspection(checked === true)}
              />
              <Label htmlFor={`requires-inspection-${order.id}`}>
                Require completion inspection before availability
              </Label>
            </div>
          )}
        </div>
        <DialogFooter>
          ...
          <Button
            disabled={!outcome.trim() || complete.isPending}
...
function CancelServiceDialog({
  ...
  const cancel = useMutation({
    mutationFn: () =>
      inventoryOperationsApi.cancelServiceOrder(order.id, {
        reason,
        idempotencyKey: commandKey('service-cancel'),
      }),
    onSuccess: async () => {
      setOpen(false);
      await refresh();
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            If work already started, the item will require inspection before it can become
            available.
          </DialogDescription>
        </DialogHeader>
...
function ServicePanel({
  ...
  const start = useMutation({
    mutationFn: (orderId: string) =>
      inventoryOperationsApi.startServiceOrder(orderId, {
        idempotencyKey: commandKey('service-start'),
      }),
    onSuccess: async () => {
      await refresh();
      toast.success('Service work started');
    },
    onError: (error) => toast.error(apiErrorMessage(error, 'Could not start service work')),
  });
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="font-medium">Cleaning, repair, and preparation</h2>
          <p className="text-sm text-muted-foreground">
            Each order owns its availability block and completion policy.
          </p>
        </div>
        <CreateServiceDialog data={data} refresh={refresh} />
      </div>
      {!data.serviceOrders.length ? (
        <Card>
          <CardContent className="p-8 text-center text-sm text-muted-foreground">
            No service work recorded.
          </CardContent>
        </Card>
      ) : (
        data.serviceOrders.map((order) => {
          const active = ['REQUESTED', 'SCHEDULED', 'IN_PROGRESS'].includes(order.status);
          return (
            <Card key={order.id}>
              <CardContent className="flex flex-wrap items-start justify-between gap-4 p-5">
                <div className="space-y-2">
                  <div className="flex gap-2">
                    <Badge>{label(order.serviceType)}</Badge>
                    <Badge variant="outline">{label(order.status)}</Badge>
                    {order.isAvailabilityBlocking && active && (
                      <Badge variant="destructive">Blocks rental</Badge>
                    )}
                  </div>
                  <p className="text-sm text-muted-foreground">
                    Requested {formatDate(order.requestedAt)}
                    {order.providerName ? ` · ${order.providerName}` : ''}
                  </p>
                  {order.notes && <p className="whitespace-pre-line text-sm">{order.notes}</p>}
                  {order.completionOutcome && (
                    <p className="text-sm">Outcome: {order.completionOutcome}</p>
                  )}
                </div>
                <div className="flex gap-2">
                  {(order.status === 'REQUESTED' || order.status === 'SCHEDULED') && (
                    <Button
                      size="sm"
                      variant="outline"
                  ...
                  {active && <CompleteServiceDialog order={order} refresh={refresh} />}
                  {active && <CancelServiceDialog order={order} refresh={refresh} />}
                </div>
              </CardContent>
            </Card>
          );
...
function ComponentStateRow({
  ...
  definition: SetComponentDefinition;
  state: StockUnitOperations['stockUnit']['componentStates'][number] | undefined;
  stockUnitId: string;
  refresh: () => Promise<void>;
  remove: () => void;
}) {
  const [presence, setPresence] = useState<StockUnitComponentPresence>(
    state?.presence || 'PRESENT',
  );
  const [quantity, setQuantity] = useState(state?.presentQuantity ?? definition.requiredQuantity);
  const update = useMutation({
    mutationFn: () =>
      inventoryOperationsApi.updateComponentState(stockUnitId, definition.id, {
        presence,
        presentQuantity: presence === 'MISSING' ? 0 : quantity,
      }),
    onSuccess: async () => {
      await refresh();
      toast.success('Component state updated');
    },
    onError: (error) => toast.error(apiErrorMessage(error, 'Could not update component state')),
  });
  const dirty =
    presence !== (state?.presence || 'PRESENT') ||
    quantity !== (state?.presentQuantity ?? definition.requiredQuantity);
  return (
    <Card>
      <CardContent className="grid items-center gap-4 p-4 md:grid-cols-[1fr_180px_120px_auto_auto]">
        <div>
          <p className="font-medium">
            {definition.name} × {definition.requiredQuantity}
          </p>
          <p className="text-xs text-muted-foreground">
            {definition.inspectionGuidance || 'No special inspection guidance'} ·{' '}
            {definition.absenceBlocksRental ? 'Required' : 'Optional'}
          </p>
        </div>
        <Select
          value={presence}
          onValueChange={(value) => setPresence(value as StockUnitComponentPresence)}
        >
          <SelectTrigger>
            <SelectValue />
          </SelectTrigger>
        ...
        <Input
          aria-label={`${definition.name} quantity`}
          type="number"
          min={0}
          value={quantity}
          onChange={(event) => setQuantity(Math.max(0, Number(event.target.value) || 0))}
        />
        <Button size="sm" disabled={!dirty || update.isPending} onClick={() => update.mutate()}>
          Save
        </Button>
        <Button
          size="icon"
...
function ComponentsPanel({
  ...
  const create = useMutation({
    mutationFn: () =>
      inventoryOperationsApi.createSetComponent(data.stockUnit.variantSize.id, {
        name,
        requiredQuantity: quantity,
        inspectionGuidance: guidance.trim() || undefined,
        absenceBlocksRental: blocking,
      }),
    onSuccess: async () => {
      setOpen(false);
      setName('');
      await refresh();
      toast.success('Set component added to this SKU');
    },
    onError: (error) => toast.error(apiErrorMessage(error, 'Could not add set component')),
  });
  const remove = useMutation({
    mutationFn: inventoryOperationsApi.deactivateSetComponent,
    onSuccess: async () => {
      await refresh();
      toast.success('Set component deactivated');
    },
    onError: (error) => toast.error(apiErrorMessage(error, 'Could not remove set component')),
  });
  const states = new Map(
    data.stockUnit.componentStates.map((state) => [state.setComponentDefinitionId, state]),
  );
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="font-medium">Set completeness</h2>
          <p className="text-sm text-muted-foreground">
            Define every piece in this SKU, then track completeness on this exact asset.
          </p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Add SKU set component</DialogTitle>
              <DialogDescription>
                Examples: blouse, dupatta, earring pair, brooch, belt, or garment bag.
              </DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-2">
              <div className="grid gap-2">
              ...
              <div className="grid gap-2">
                <Label>Inspection guidance</Label>
              ...
              <div className="flex items-center gap-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setBlocking(checked === true)}
                />
                <Label htmlFor="absence-blocks">Missing or damaged component blocks rental</Label>
              </div>
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setOpen(false)}>
      ...
      {!data.stockUnit.variantSize.setComponentDefinitions.length ? (
        <Card>
          <CardContent className="p-8 text-center text-sm text-muted-foreground">
            This SKU is currently a single-piece item. Add components only when the rentable unit is
            a set.
          </CardContent>
        </Card>
      ) : (
        data.stockUnit.variantSize.setComponentDefinitions.map((definition) => (
          <ComponentStateRow
            key={definition.id}
            ...
            stockUnitId={data.stockUnit.id}
            refresh={refresh}
            remove={() => remove.mutate(definition.id)}
          />
        ))
      )}
    </div>
...
function Overview({ data, refresh }: { data: StockUnitOperations; refresh: () => Promise<void> }) {
  ...
  const activeService = data.serviceOrders.filter(
    (order) =>
      order.isAvailabilityBlocking &&
      ['REQUESTED', 'SCHEDULED', 'IN_PROGRESS'].includes(order.status),
  );
  const incompleteComponents = unit.componentStates.filter(
    (state) =>
      state.setComponentDefinition.absenceBlocksRental &&
      (state.presence === 'MISSING' ||
        state.presence === 'DAMAGED' ||
        state.presentQuantity < state.setComponentDefinition.requiredQuantity),
  );
  return (
    <div className="space-y-4">
      {(blockingIssues.length > 0 ||
        ...
        <Alert variant="destructive">
          ...
          <AlertTitle>Not ready for availability</AlertTitle>
          <AlertDescription>
            {blockingIssues.length} blocking issue(s), {activeService.length} active blocking
            service order(s), and {incompleteComponents.length} incomplete required component(s).
          </AlertDescription>
        </Alert>
      )}
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardTitle className="text-lg">{label(unit.condition)}</CardTitle>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>Active assignments</CardDescription>
            <CardTitle className="text-lg">{unit.assignments.length}</CardTitle>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardTitle className="text-lg">
              {data.issues.filter((issue) => ['OPEN', 'IN_SERVICE'].includes(issue.status)).length}
            </CardTitle>
          </CardHeader>
        </Card>
        <Card>
      ...
      {unit.assignments.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Current and future assignments</CardTitle>
          </CardHeader>
            ...
            {unit.assignments.map((assignment) => (
              ...
              >
                <Link
                  ...
                  href={`/dashboard/bookings/${assignment.reservation.bookingId}`}
                >
                  Booking {assignment.reservation.bookingId.slice(0, 8)}
                </Link>
                <span className="text-muted-foreground">
                  {assignment.blockedStartDate.slice(0, 10)} →{' '}
                  {assignment.blockedEndDate.slice(0, 10)}
                </span>
              </div>
            ))}
          </CardContent>
...
export default function StockUnitOperationsPage() {
  ...
  const unitQuery = useQuery({
    ...
    queryFn: () => inventoryOperationsApi.getUnit(stockUnitId),
    enabled: !!stockUnitId,
  });
  const refresh = async () => {
    await Promise.all([
    ...
    return (
      <div className="space-y-4">
        <Button variant="ghost" asChild>
          <Link href={`/dashboard/products/${productId}/inventory`}>
        ...
        <Alert variant="destructive">
          ...
          <AlertTitle>Physical item could not be loaded</AlertTitle>
          <AlertDescription>
            {apiErrorMessage(
              unitQuery.error,
              'The item may no longer exist or you may not have access.',
            )}
          </AlertDescription>
        </Alert>
      </div>
    ...
    <div className="space-y-6 pb-12">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="space-y-1">
          ...
          <h1 className="font-mono text-2xl font-semibold tracking-tight">{unit.assetCode}</h1>
          <p className="text-sm text-muted-foreground">
            {unit.variantSize.variant.product.name} ·{' '}
            {unit.variantSize.variant.variantName || unit.variantSize.variant.mainColor.name} · Size{' '}
            {unit.variantSize.sizeInstance.displayLabel}
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Badge variant={availabilityVariant(unit)}>{label(unit.disposition)}</Badge>
          <Badge variant="outline">{label(unit.operationalState)}</Badge>
          <Badge variant="outline">{label(unit.condition)}</Badge>
          {blockingCount > 0 && (
      ...
      <Tabs defaultValue="overview" className="space-y-4">
        ...
        <TabsContent value="service">
          <ServicePanel data={data} refresh={refresh} />
        </TabsContent>
        <TabsContent value="components">
          <ComponentsPanel data={data} refresh={refresh} />
        </TabsContent>
              ...
              data.lifecycleEvents.map((event) => (
                <Card key={event.id}>
                  <CardContent className="flex flex-wrap items-start justify-between gap-3 p-4">
                    <div>
                      <p className="text-sm font-medium">
                        {label(event.fromDisposition)} / {label(event.fromOperationalState)} →{' '}
