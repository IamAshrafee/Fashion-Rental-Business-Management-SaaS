...
import { AlertCircle, ArrowLeft, Boxes, Loader2, PackagePlus, Trash2 } from 'lucide-react';
...
function RuleDialog({ productId, products, existing, onSaved }: { productId: string; products: ProductListItem[]; existing?: CompositionRule; onSaved: () => Promise<void> }) {
  ...
  const save = useMutation({
    mutationFn: () => {
      const payload: CompositionRuleInput = {
        ...
        substitutionPolicy,
        pricingBehavior,
        priceAdjustment: pricingBehavior === 'INCLUDED' ? 0 : priceAdjustment,
        allocationWeight: 1,
        isDefaultSelected: role === 'OPTIONAL_ADDON' && resolution === 'FIXED' && defaultSelected,
  ...
  const valid = Boolean(
    name.trim() &&
    componentProductId &&
    (resolution !== 'FIXED' || fixedVariantSizeId) &&
    (pricingBehavior === 'INCLUDED' || priceAdjustment > 0),
  );
  ...
  return <Dialog open={open} onOpenChange={setOpen}>
    ...
    <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
      ...
      <div className="grid gap-4 py-2">
        ...
        {pricingBehavior !== 'INCLUDED' && <div className="grid gap-2"><Label>Price adjustment (৳)</Label><Input type="number" min="0.01" step="0.01" value={minorToMajorInput(priceAdjustment)} onChange={(event) => setPriceAdjustment(majorInputToMinor(event.target.value) ?? 0)} /><p className="text-xs text-muted-foreground">A positive charge is required for additive or optional pricing.</p></div>}
        <div className="grid gap-2"><Label>Internal compatibility notes</Label><Textarea maxLength={1000} value={compatibilityNotes} onChange={(event) => setCompatibilityNotes(event.target.value)} placeholder="Instructions for matching color, size, style, or material" /><p className="text-xs text-muted-foreground">Operational guidance for staff; parent-derived resolution enforces exact normalized size matching.</p></div>
        ...
        <div className="flex flex-wrap gap-5">{role === 'OPTIONAL_ADDON' && resolution === 'FIXED' ? <label className="flex items-center gap-2 text-sm"><Checkbox checked={defaultSelected} onCheckedChange={(checked) => setDefaultSelected(checked === true)} />Selected by default</label> : null}<label className="flex items-center gap-2 text-sm"><Checkbox checked={customerApproval} onCheckedChange={(checked) => setCustomerApproval(checked === true)} />Customer approval for substitution</label></div>
      </div>
...
export default function ProductCompositionPage() {
  ...
  const productQuery = useQuery({ queryKey: ['products', 'detail', id], queryFn: () => productApi.getById(id), enabled: !!id });
