...
| Availability calculation | PARTIAL | Authoritative pooled/serialized capacity includes policy buffers, locations, condition/state eligibility, blocks, components, issues, reservations, assignments, transfers, bundles, and internal date-filter reuse with unit/PostgreSQL coverage. | Next-available guidance, quote freshness, and final public/cache/timezone contract close in D/F. | D1/F2 |
